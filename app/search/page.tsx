      ...
      <div className='w-full flex flex-col justify-center text-left px-16 mt-10 font-work-sans'>
        {data.data.map((item: article)=>{
            return (
                    ...
                    <Link href={{pathname:`/article/${item['_id']}`}} key={item['_id']}>
                    <div  className='max-w-4xl flex flex-col justify-center my-3'>
                        ...
                        <p className='text-lg sm:text-xl font-semibold grid grid-cols-4 lg:grid-cols-6 w-full'>
                            {item['authors'].map((author) => {return (<span className='italic mx-3' key={author._id}>{author.name}</span>)})}
                        </p>
                        <p className='text-sm lg:text-lg font-medium whitespace-nowrap overflow-hidden text-ellipsis'>{item['abstract'] }</p>
                        <p className='text-sm lg:text-lg font-semibold grid grid-cols-4 sm:grid-cols-6'>
                            {item['keywords'].map((keyword, index) => {return (<span className='underline m-1' key={index}>#{keyword}</span>)})}
                        </p>
                    </div>
                    </Link>
